...
namespace LookingGlass {
	...
	public class Holoplay : MonoBehaviour {
		...
		public const string versionLabel = "b9";
		...
		public bool automaticWindowManagement = true;
		...
		#if UNITY_POST_PROCESSING_STACK_V2
		DepthOfField ppDof;
		bool ppProfileWithDof;
		public float dofAmount = 12f;
		#endif
		...
		void OnEnable() {
			...
			if (ppVolume != null) {
				...
				ppProfileWithDof = ppProfile.TryGetSettings<DepthOfField>(out ppDof);
			}
			...
			if (!Application.isEditor && automaticWindowManagement) {
				Display.displays[0].Activate(0, 0, 0);
				Display.displays[0].SetParams(cal.screenWidth, cal.screenHeight, Plugin.GetLKGxpos(0), Plugin.GetLKGypos(0));
	...
	#if UNITY_POST_PROCESSING_STACK_V2
			if (ppProfileWithDof) {
				ppDof.focusDistance.value = Mathf.Abs(camDist);
				ppDof.focalLength.value = Mathf.Abs(camDist * dofAmount);
				ppDof.aperture.value = Mathf.Abs(camDist / dofAmount);
			}
	#endif
		}
        ...
        public void RenderQuilt (bool forceRender = false) {
			...
			// GL.Clear(true, true, new Color(background.r, background.g, background.b, 0f), 0f);
			...
			RenderTexture quiltRTDepth)
		{
			...
			for (int i = 0; i < quiltSettings.numViews; i++) {
				...
				if (i != 0 && i != quiltSettings.numViews - 1 && i % ViewInterpolation != 0) {
					...
					float offset = div * Mathf.Tan(cal.viewCone * Mathf.Deg2Rad) / (quiltSettings.numViews - 1f);
